...
public class testRegistrarse extends ActivityInstrumentationTestCase2<MainActivity> {
	...
	public void testRun() {
		...
		//Click on Empty Text View
		solo.clickOnView(solo.getView(com.GPS.app.fooding.R.id.nombre));
		//Enter the text: 'holaquetal'
		solo.clearEditText((android.widget.EditText) solo.getView(com.GPS.app.fooding.R.id.nombre));
		solo.enterText((android.widget.EditText) solo.getView(com.GPS.app.fooding.R.id.nombre), "prueba");
        //Click on Registrarse
		RegistrarseFragment.test = true;
		solo.clickOnView(solo.getView(com.GPS.app.fooding.R.id.RegistrarseButton));
